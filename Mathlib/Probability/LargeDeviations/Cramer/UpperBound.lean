...
lemma Cramer.limsup_inv_mul_log_measure_nonpos (s : ℕ → Set Ω) :
    limsup (fun n : ℕ => ((1 : ℝ) / (n : ℝ) : EReal) * ENNReal.log (μ (s n))) atTop ≤ 0 :=
  limsup_le_of_le (isCoboundedUnder_le_of_le atTop fun _ => bot_le) <|
    Eventually.of_forall fun n => EReal.mul_nonpos_iff.mpr <| Or.inl
...
When the supremum defining the rate function is unbounded, `rateFunction X μ a` takes the junk
...
theorem Cramer.limsup_le_neg_rateFunction (a : ℝ) (h_mean : μ[X 0] ≤ a) :
  ...
  by_cases hbdd : BddAbove (Set.range fun t => t * a - cgf (X 0) μ t)
  · unfold rateFunction
  ...
  · -- The supremum is unbounded: `rateFunction X μ a = 0` by the `Real.iSup` junk-value
    -- convention, and the scaled log-probabilities are nonpositive.
    rw [rateFunction, Real.iSup_of_not_bddAbove hbdd, EReal.coe_zero, neg_zero]
